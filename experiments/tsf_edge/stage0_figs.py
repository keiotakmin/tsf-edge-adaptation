...
# TERMINOLOGY (2026-09-03, see ieee_access/GLOSSARY.md).  The paper calls the held-out
...
def lr_response_paper():
    ...
    y0, dy = -34, 9
    xlab = min(lrs) * 0.78
    axA.text(xlab, y0 + 8, "cells $<$ static:", fontsize=5.5, color="0.3", va="center")
    for i, (lab, col, neg) in enumerate(neg_rows):
        y = y0 - i * dy
        axA.text(xlab, y, lab.replace("full-", ""), fontsize=5.5, color=col,
                 va="center", ha="left")
        for x, c in zip(lrs, neg):
            if c:
                axA.text(x, y, f"{c}", fontsize=5.5, color=col, ha="center", va="center")
    axA.axhline(0, color="0.4", lw=0.8)
    axA.axvline(1e-3, color="0.25", ls="--", lw=1.0)
    axA.set_xscale("log")
    axA.set_xlim(min(lrs) * 0.7, max(lrs) * 1.5)   # tracks the rates actually drawn, so
...
TRADEOFF = [
    ...
    ("obsign_t3e3",   "ObSign $\\tau$=3e-3", "new", "H"),
    ("obsign_t1e3",   "ObSign $\\tau$=1e-3", "new", "d"),
]
...
ABLATIONS = [
    ("obsign",  "ObSign $\\tau$=1e-2 (guard too loose)", "new", "8"),
...
def optimizer_table():
    ...
    spec = [t for t in TRADEOFF + ABLATIONS if t[0] != "sgd_default"]
    ...
    tex = [r"% AUTO-GENERATED by stage0_figs.optimizer_table() -- do not edit.",
           ...
           r"\caption{Every arm on the same 216 cells, under both readings. \emph{state} is "
           r"optimizer state per trainable parameter; adaptation memory is "
           r"$4N(1+\mathit{state})$ bytes, with $N=" + f"{n_ref:,}".replace(",", r"{,}") +
           r"$ at PatchTST, $L{=}96$, $H{=}24$. The \emph{tuned} columns select the online "
           r"rate on the held-out pre-drift slice of the shared grid; the "
           r"\emph{at $10^{-3}$} columns are the same cells with no tuning at all, which is "
           r"what a device that cannot afford a tuning pass actually gets. \emph{neg} counts "
           r"cells below the static baseline; diverged cells (benefit $<-100\%$) are excluded "
           r"from the means and from \emph{worst} but still counted in \emph{neg}. "
           r"\emph{mis1x} is the benefit lost to a one-decade error in the rate, measured from "
           r"each arm's own optimum. $\dagger$ marks the state-axis Pareto frontier.}",
...
DEPLOY_TABLE_ARMS = [
    ...
    ("obsign_t1e3", r"ObSign $\tau$=1e-3"), ("adafactor", "AdaFactor"),
    ("obsign_t3e3", r"ObSign $\tau$=3e-3"), ("relsign", r"RelSign (no guard)$^\ddagger$"),
...
def requirement_table():
    ...
    # Counts quoted in the caption are computed, not typed: this file is generated, and a
    # literal "216 cells" or "ten arms" here goes quietly stale the first time the grid moves.
    n_weak = sum(1 for st in pool.fixed(list(keys), 1e-3, cells, ref).values()
                 if st["neg"] == 0)
    step_dec = np.log10(DEPLOY_TABLE_LRS[-1] / DEPLOY_TABLE_LRS[0]) / (len(DEPLOY_TABLE_LRS) - 1)
    tex = [r"% AUTO-GENERATED by stage0_figs.requirement_table() -- do not edit.",
           r"\begin{table*}[t]", r"\centering", r"\scriptsize",
           r"\caption{\textbf{What a deployment gets at each rate it might have shipped, and "
           r"never tunes.} Every entry is mean adaptation benefit \% / how many of the "
           + f"{len(cells)}" + r" cells ended up \emph{worse} than the frozen model. A "
           r"\colorbox[gray]{0.88}{shaded} cell marked $\bullet$ is one a deployment could "
           r"ship: it leaves no cell below the frozen model, \emph{and} what it gives up "
           r"against the best tuned rule is under " + f"{pool.COMPETITIVE_MARGIN:g}" + r" points "
           r"with 95\% confidence. \textbf{The band is the longest unbroken run of shaded "
           r"cells in a row}, i.e.\ the whole range of rates that could have been chosen "
           r"before seeing the site without regret. It is given twice: as a count of rates, "
           r"which the reader can check against the row, and as its width in decades, which is "
           r"what a deployment experiences. The two are not interchangeable -- the rates here "
           r"are " + f"{step_dec:.1f}" + r" decades apart, so a run of $n$ of them spans $"
           + f"{step_dec:.1f}" + r"(n{-}1)$ decades and a single rate spans none. A wide band "
           r"means nobody has to pick the rate; a one-cell band means the rule works at "
           r"exactly the rate it happens to ship with. Two notes on the criterion: the "
           r"comparison against the tuned rule is \emph{paired} per cell, because every rule "
           r"is read on the same cells and the uncertainty in a difference is far smaller than "
           r"in either mean; and harmlessness alone is passed by " + f"{n_weak}"
           + r" of the rules listed here, plain SGD among them, which is why being competitive "
           r"is part of the criterion at all. Only class representatives are listed; every "
           r"other rule in Table~\ref{tab:optimizers} has a band of one rate or none. "
           r"$\ddagger$ RelSign has no learning rate -- its lr \emph{is} $\tau$ -- so its row "
           r"reads sensitivity to its own constant, which is what the ablation asks.}",
           r"\label{tab:requirements}",
           r"\begin{tabular}{lr" + "r" * len(DEPLOY_TABLE_LRS) + "rr}", r"\toprule",
           r"& & \multicolumn{" + str(len(DEPLOY_TABLE_LRS))
           + r"}{c}{shipped default learning rate} & \multicolumn{2}{c}{band} \\",
           r"\cmidrule(lr){3-" + str(2 + len(DEPLOY_TABLE_LRS)) + r"}\cmidrule(lr){"
           + str(3 + len(DEPLOY_TABLE_LRS)) + "-" + str(4 + len(DEPLOY_TABLE_LRS)) + "}",
           r"optimizer & state & " + " & ".join(hdr) + r" & rates & decades \\", r"\midrule"]
    labels = dict(DEPLOY_TABLE_ARMS)
    for k in order:
        d = D[k]
        tex.append(f"{labels[k]} & ${d['state']:.2f}" + r"\times$ & "
                   + " & ".join(cell(r) for r in d["rows"])
                   + f" & {d['band']} & " + ("---" if d["band"] < 2
                                              else f"{d['band_decades']:.1f}") + r" \\")
    tex += [r"\bottomrule", r"\end{tabular}", r"\end{table*}", ""]
...
def requirement_gap_paper():
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(TEXTWIDTH, 4.4), sharex=True,
    ...
    ax1.text(2.42e5, (best_zero + best_any) / 2, f"{best_any - best_zero:.1f} pt",
             fontsize=6.8, color="#a01f1f", ha="left", va="center", zorder=7)
    ax1.text(xhi * 0.97, best_any + 0.35, "best untuned benefit at any budget (AdaFactor)",
             fontsize=6.2, color="0.4", ha="right", va="bottom")
    ...
    ax2.text(xhi * 0.97, 1.3, "static model (0%)", fontsize=6.6, color="0.2", ha="right",
             va="bottom")
    ax2.text(xlo * 1.04, -22.6, "below here, adapting is worse than\nleaving the model alone",
             fontsize=6.4, color="#a01f1f", ha="left", va="bottom")
    ...
    for ax, span in ((ax1, 12.3), (ax2, 26.7)):
        ...
        for x, members in cols.items():
            ...
            for side, mem in (("L", members[0::2]), ("R", members[1::2])):
                if not mem:
                    continue
                ys = _spread_labels([S[k]["mean"] for k in mem], 0.105 * span)
                left = side == "L"
                for k, ly in zip(mem, ys):
                    ax.annotate(lab_of[k], xy=(x, S[k]["mean"]),
                                xytext=(x * (0.88 if left else 1.13), ly), fontsize=6.4,
    ...
    ax2.set_xlabel("adaptation memory: grads + opt. state (B, log scale, $\\downarrow$ better)\n"
                   "quoted at PatchTST, $L$=96, $H$=24")
    fig.text(0.012, 0.55, "adaptation benefit % with NO rate tuning\n"
    ...
    fig.legend(handles=handles, loc="upper center", ncol=4, bbox_to_anchor=(0.5, 1.025),
               frameon=False, fontsize=6.6, handletextpad=0.4, columnspacing=1.4)
    ...
    fig.text(0.5, 0.945, "( ) after each name: cells below the static model",
             ha="center", fontsize=7.0, color="0.35")
    fig.tight_layout(rect=(0.03, 0, 1, 0.925))
...
def knee_paper():
    ...
    import stage0_pool as P
    cells, ref = P.load_cells()
    arms = [a for _, a, _, _ in KNEE_TAUS] + ["signsgd"]
    ...
    axA.text(6.0e-3, _cap * 0.62, "above the knee: lr cancels,\n$\\alpha$ fixed at "
             "$\\tau\\,$RMS$(p)$", fontsize=6.4, color="#333333", ha="center", va="top")
    ...
    axA.set_title("(A) the effective step $\\alpha=\\min($lr$,\\ \\tau\\,$RMS$(p))$\n"
    ...
    axB.axvspan(XLO, 1.0, color="#0072B2", alpha=0.09, zorder=0)
    axB.text(1.42, -13.8, "design rule:\nmargin $\\geq$ 1 decade", fontsize=6.4,
             color="#1b5e86", ha="left", va="bottom")
    axB.axvline(1.0, color="#0072B2", lw=1.0, ls="--", zorder=1)
    axB.axvspan(XSEP, XHI, color="0.85", alpha=0.55, zorder=0)
    axB.text(XINF, 17.4, "guard\nremoved", fontsize=6.2, color="0.35", ha="center", va="top")
    ...
    for a, x, y in zip(order, xs, ys):
        axB.scatter(x, y, s=70, color="#333333" if ok_of[a] else "w", edgecolor="#333333",
                    lw=1.1, zorder=4)
        axB.annotate(f"{tau_label(a)}\n({F[a]['neg']})", xy=(x, y), xytext=(0, -11),
                     textcoords="offset points", fontsize=6.2, ha="center", va="top",
                     color="0.12")
    ...
    axB.text(XLO - 0.03, 0.6, "static model (0%)", fontsize=6.2, color="0.2", ha="left",
             va="bottom")
    axB.set_xlim(XLO, XHI); axB.set_ylim(-15, 19)
