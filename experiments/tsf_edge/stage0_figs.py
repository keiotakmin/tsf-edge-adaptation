...
def optimizer_table():
    ...
    tex = [r"% AUTO-GENERATED by stage0_figs.optimizer_table() -- do not edit.",
           ...
           r"\caption{Every arm on the same " + f"{len(cells)}" + r" cells, under both "
           r"readings of Section~\ref{sec:reqs}. \emph{state} is optimizer state per trainable "
           r"parameter; adaptation memory is $4N(1+\mathit{state})$ bytes, with $N="
           + f"{n_ref:,}".replace(",", r"{,}") + r"$ at PatchTST, $L{=}96$, $H{=}24$. "
           r"\emph{Tuned} selects the online rate on the held-out pre-drift slice; "
           r"\emph{at $10^{-3}$} is the same cells with no tuning at all. \emph{neg} counts "
           r"cells below the static baseline, diverged ones (benefit $<-100\%$) included, "
           r"though those are left out of the means and of \emph{worst}. \emph{mis1x} is the "
           r"benefit lost to a one-decade error in the rate, from each arm's own optimum. "
           r"$\dagger$ marks the state-axis Pareto frontier.}",
...
def requirement_table():
    ...
    tex = [r"% AUTO-GENERATED by stage0_figs.requirement_table() -- do not edit.",
           ...
           r"\colorbox[gray]{0.88}{shaded} cell marked $\bullet$ is \emph{deployable} in the "
           r"sense of requirement R3, and \textbf{the band is the longest unbroken run of "
           r"shaded cells in a row}: the whole range of rates that could have been chosen "
           r"before seeing the site without regret. It is given as a count of rates, which the "
           r"reader can check against the row, and as its width in decades; both, and the "
           r"paired test behind the shading, are defined in Section~\ref{sec:reqs}. Only class "
           r"representatives are listed; every other rule in Table~\ref{tab:optimizers} has a "
           r"band of one rate or none. $\ddagger$ RelSign has no learning rate -- its lr "
           r"\emph{is} $\tau$ -- so its row reads sensitivity to its own constant, which is "
           r"what the ablation asks.}",
